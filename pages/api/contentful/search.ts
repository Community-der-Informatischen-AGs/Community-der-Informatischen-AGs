...
export default async function handler(
  ...
  try {
    ...
    const entryCollectionTypeOrderToQueryLimitKeys = Object.keys(entryCollectionTypeOrderToQueryLimit);
    ...
    for (let key of entryCollectionTypeOrderToQueryLimitKeys) {
      ...
      entryCollectionTypeOrderToQueryLimit[key] = resultCount;
  
    }
  
    let currentResultSum = Object.values(entryCollectionTypeOrderToQueryLimit).reduce((a, b) => a + b);
    let keyIndex = 0;
    while (currentResultSum > 5) {
      let key = entryCollectionTypeOrderToQueryLimitKeys[keyIndex];
      if (entryCollectionTypeOrderToQueryLimit[key] > 1) {
        entryCollectionTypeOrderToQueryLimit[key] = entryCollectionTypeOrderToQueryLimit[key] - 1;
      }
      keyIndex++;
    }
    ...
    for (let key of entryCollectionTypeOrderToQueryLimitKeys) {
      ...
      const limit = entryCollectionTypeOrderToQueryLimit[key];
      
      if (limit <= 0) {
        continue;      
      }
  
      const queryResult = await Contentful.getAllOfEntryCollection(
        key,
